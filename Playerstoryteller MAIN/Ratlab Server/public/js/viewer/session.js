...
export function renderSessionsList() {
    ...
    STATE.sessions.forEach(session => {
        ...
        const region = session.region || 'US-EAST';

        div.innerHTML = `
        <div class="session-card group bg-rat-panel border border-rat-border hover:border-rat-green rounded-lg p-6 cursor-pointer transition-all relative overflow-hidden" onclick="window.selectSession('${session.sessionId}')">
            
            <div class="flex justify-between items-start mb-4 relative z-10">
                <div>
                    <h3 class="font-bold text-xl text-white group-hover:text-rat-green transition-colors">${session.mapName || 'Unknown Colony'}</h3>
                    <div class="text-xs font-mono text-rat-text-dim mt-1">ID: ${session.sessionId}</div>
                </div>
            ...
            <div class="grid grid-cols-2 gap-4 text-sm font-mono relative z-10">
                <div>
                    ...
                    <div class="text-white">${session.colonistCount} Subjects</div>
                </div>
                 <div>
                    <div class="text-rat-text-dim text-xs">WEALTH</div>
                    <div class="text-white">$${wealth}</div>
                </div>
                 <div>
                    <div class="text-rat-text-dim text-xs">VIEWERS</div>
                    <div class="text-white">${playerCount} Watching</div>
                </div>
                <div>
                    <div class="text-rat-text-dim text-xs">UPTIME</div>
                    <div class="text-white">${session.uptime || '00:00:00'}</div>
                </div>
            </div>
            ...
            if (data.session) {
                ...
                if(nameEl) {
                    if (STATE.sessionRequiresPassword) {
                        nameEl.innerHTML = `${data.session.mapName} <i class="fa-solid fa-lock text-rat-red text-xs ml-2"></i>`;
                    } else {
