...
if (mapContent) {
    ...
    mapContent.addEventListener('wheel', (e) => {
        ...
        mapScale *= delta;
        mapScale = Math.min(Math.max(0.5, mapScale), 5); // Clamp 0.5x to 5x
        updateMapTransform();
    });
...
document.getElementById('map-zoom-in')?.addEventListener('click', () => {
    mapScale = Math.min(mapScale + 0.5, 5);
    updateMapTransform();
});

document.getElementById('map-zoom-out')?.addEventListener('click', () => {
    mapScale = Math.max(mapScale - 0.5, 0.5);
    updateMapTransform();
});
...
function updateColonistsList(newColonists, gameState) {
...
function updateMedicalAlertsList(newMedicalAlerts) {
