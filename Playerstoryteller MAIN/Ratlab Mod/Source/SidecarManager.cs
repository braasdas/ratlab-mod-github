...
namespace PlayerStoryteller
{
    ...
    public class SidecarManager
    {
        ...
        private string GetSidecarScriptPath()
        {
            ...
            // Look for 'go-sidecar/sidecar.exe' (window capture version) in the mod root
            string path = Path.Combine(modContent.RootDir, "go-sidecar", "sidecar.exe");
            return path;
        }
        ...
        public void Start(string sessionId, string streamKey)
        {
            ...
            try
            {
                ...
                {
                    FileName = exePath,
                    Arguments = $"{sessionId} --stream-key {streamKey} --parent-pid {rimWorldPid} --capture-mode window --server-url {wsUrl} --quality {PlayerStorytellerMod.settings.streamingQuality}",
